...
function boot(id,{w=736,h=500,reduced=false,embedded=false,hash=''}={}){
  ...
  for(const m of html.matchAll(/<([a-z][a-z0-9-]*)\b[^>]*\bid="([^"]+)"/g)){const el=new Element(m[1]);el.id=m[2];nodes[m[2]]=el;}
